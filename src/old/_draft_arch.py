...
class HyenaDNAHF(nn.Module):
    ...
    model = HyenaDNAHF.from_pretrained(
        path="/g/data/te53/en9803/data/hf-cache/models",
        model_name="hyenadna-small-32k-seqlen-hf",
        device="cuda",
        use_head=False,
    ...
    def _graft_backbone(
        ...
        *,
        ...
        for key in list(scratch_dict.keys()):
            ...
            lookup = f"model.{key}"
            if checkpointing:
                lookup = cls._inject_substring(lookup)

            if lookup in pretrained_dict:
                scratch_dict[key] = pretrained_dict[lookup]
                hits += 1
            else:
                alt = cls._inject_substring(key) if checkpointing else key
                if alt in pretrained_dict:
                    scratch_dict[key] = pretrained_dict[alt]
                    hits += 1
                else:
                    misses.append((key, lookup))

        if verbose:
            print(f"[hyena] grafted backbone: {hits} params; missing: {len(misses)}")
            if misses and strict:
                for k, lk in misses[:20]:
                    print(f"  miss: scratch='{k}' searched='{lk}'")
        if misses and strict:
...
enc = backend_hy.tokenizer(
    ...
    truncation=True,
    max_length=backend_hy.max_length,  # if you’re NOT auto-adding specials, don’t +2
    return_tensors="pt",
)
...
with torch.inference_mode(), torch.autocast("cuda", dtype=backend_hy.torch_dtype):
    out = backend_hy.model(input_ids, output_hidden_states=True, return_dict=True)   # DO NOT unsqueeze
    hs = out.hidden_states
    print(len(hs), hs[0].shape, hs[-1].shape)
    # Example: take layer k (including 0=embeddings, -1=final) and pool
    k = 0
    pooled1 = hs[k].mean(dim=1)
    k = -1
    pooled2 = hs[k].mean(dim=1)
    # If you need your own mask for pooling:
